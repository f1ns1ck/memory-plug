...
export async function autoCaptureChange(input) {
    ...
    try {
        ...
        const windowMs = typeof index.coalesce_window_ms === "number"
    ...
    const result = await runCapture({ projectPath: projectRoot, reason: autoReason(input) }, wt, { replaceChangeId });
    if (!result.captured) {
