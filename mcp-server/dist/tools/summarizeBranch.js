 ...
 * read, never written.
 */
export async function summarizeBranch(input) {
    ...
    if (!target) {
        return ("Could not determine a branch to summarize. Pass a branch name, or run " +
    ...
    if (!onBranch.length) {
        return (`No recorded changes on branch \`${target}\`.\n\n` +
    ...
    if (commits.length)
        meta.push(`commits: ${commits.join(", ")}`);
    if (authors.length)
        meta.push(`by ${authors.join(", ")}`);
    ...
    if (onBranch.length > changes.length) {
        lines.push(`_Showing the ${changes.length} most recent of ${onBranch.length} changes on this branch._`);
    }
    return lines.join("\n").trimEnd() + "\n";
}
//# sourceMappingURL=summarizeBranch.js.map