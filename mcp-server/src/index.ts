...
const tools = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        sharePatches: {
          type: "boolean",
          description:
            "Opt-in: commit patches/ so teammates can load any change's diff. Re-run to toggle (default OFF — patches stay machine-local).",
        },
      },
    },
  ...
  },
  {
    ...
    description:
      "Automatically capture the current working-tree diff after Claude edits files. Debounced and deduplicated for hook usage. Wrapper over capture_change: skips on clean tree, duplicate diff, or within the debounce window. Read-only git; never commits or modifies the repo.",
    inputSchema: {
      type: "object",
      properties: {
        ...projectPathProp,
    ...
    handler: (a: any) => autoCaptureChange(a ?? {}),
  },
  {
    name: "set_auto_capture",
    description:
      "Turn automatic capture on or off for this machine (per-developer, stored in the local auto-capture.json — not committed). Omit 'enabled' to report the current state.",
    inputSchema: {
      type: "object",
      properties: {
        ...projectPathProp,
    ...
    handler: (a: any) => setAutoCapture(a ?? {}),
  },
  {
    name: "set_share_patches",
    description:
      "Turn patch sharing on or off for this project (team decision, stored as share_patches in the committed index.json). ON commits patches/ so teammates can load any change's diff; OFF keeps patches machine-local. Regenerates the managed .gitignore. Omit 'enabled' to report the current state.",
    inputSchema: {
      type: "object",
      properties: {
        ...projectPathProp,
    ...
    handler: (a: any) => setSharePatches(a ?? {}),
  },
  {
