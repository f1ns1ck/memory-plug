...
export interface WorkingTreeDiff {
  /** Full composed patch: tracked diff + synthetic blocks for untracked files. */
  diff: string;
  /** All changed files (tracked + untracked), POSIX-style, de-duplicated. */
  files: string[];
  /** Per-file add/modify/remove status. */
  nameStatus: { status: string; file: string }[];
 ...
 */
async function untrackedDiffBlock(
  projectRoot: string,
  relFile: string,
): Promise<string> {
