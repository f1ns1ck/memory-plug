...
export function estimateTokensOf(...parts: string[]): number {
  return estimateTokens(parts.join("\n"));
}

export interface TrimOptions {
  maxTokens: number;
  /** Minimum number of recent changes to keep even when over budget. */
  minRecentChanges?: number;
}
 ...
 */
export function fitSections(sections: string[], maxTokens: number): string[] {
  const kept: string[] = [];
  let running = 0;
