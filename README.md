...
- **Share patches too (opt-in).** Run `/memory share on` (or
  `configure({ sharePatches: true })`) to commit `patches/` as well, so
...
`Write`, `Edit` and `MultiEdit` tools and runs the plugin's auto-capture command
(`mcp-server/dist/cli/autoCapture.js`) directly — no MCP round-trip.
...
- Run **`/memory auto off`** (calls `configure`). The flag is stored in the
